...
export const projects: Project[] = ([
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "Suica 등 일본 현지에서 널리 쓰이는 FeliCa 규격의 모바일 스마트카드를 단말 리더기가 인식했을 때, 로컬 데이터베이스를 기반으로 단말 자체에서 1대N 검증을 신속히 수행하는 하드웨어 리딩 최적화",
        challenge:
      ...
      },
      {
        ...
        description:
          "인증 완료 시 외부 도어 컨트롤러 또는 하드웨어 컨트롤 장치로 Wiegand 신호를 송신하거나 리더로부터 카드 번호를 수신할 때, 비트 수준의 신호 정합성을 보장하는 임베디드 통신 최적화",
        challenge:
          "이기종 단말 모델(NPro와 XPro) 간에 Wiegand 비트 전송 규격이 달라, 타사 컨트롤러 연동 시 카드 ID의 이진 변환 값이 비트 밀림 등으로 깨져 출입문 제어가 무산되는 장애 발생.",
        solution:
  ...
  },
  {
    ...
    result:
      "Android 앱부터 전용 서버, 인증, 결제, 푸시, CI/CD까지 직접 설계하고 Google Play Closed Testing에서 핵심 결제·알림 흐름 검증 완료 (프로덕션 공개 배포 준비 중).",
    metrics: [
      {
        label: "모듈 아키텍처",
        value: "4-Module",
    ...
    features: [
      ...
      },
      {
        ...
        solution:
          "Next.js App Router 기반 BFF 서버를 구축하고 Google Cloud Pub/Sub Webhook 및 Play Developer API를 연동하여 서버 사이드에서 실시간 구매 토큰 검증 및 구독 상태 갱신 처리.",
        outcome:
    ...
    features: [
      {
        ...
        outcome:
          "Compose Screen 13개, ViewModel 8개, UseCase 21개를 분리하여 독립적인 기능 추가 및 유지보수 기반 마련."
      },
      {
  ...
  },
  {
    ...
    result:
      "기존 6클래스 기준 ACER 0.05%(단일 IR Validation) 및 0.60%(독립 Test split) 달성, 고정 IR INT8 모델의 실물 NPU invoke 지연 P50 10–11ms 검증 완료 (현재 12클래스 파이프라인 수치는 별도 Validation 범위로 관리).",
    metrics: [
      {
        label: "단일 IR ACER",
        value: "0.05%",
    ...
    features: [
      {
        ...
        solution:
          "물리적 피실험자 매핑 기반 Subject Group K-Fold를 강제 적용해 데이터 누수를 차단하고, Conv1 가중치 전이 모드를 'mean'에서 'sum'으로 변경하여 초기 활성화 스케일 왜곡 방지.",
        outcome:
      ...
      },
      {
        ...
        challenge:
          "초기 PyTorch MobileNetV3 경로에서는 Hardswish 활성화 함수와 SE 블록으로 인해 양자화 시 활성화 분포가 무너져 ACER가 50%까지 폭락하는 구조적 한계 확인.",
        solution:
      ...
      },
      {
        ...
        solution:
          "AI를 활용하여 100회 캡처 스케줄, LIVE 79.9% Rejected vs 80.0% Allowed 판정 경계, 24-bit BMP 헤더·4바이트 row padding·바이트 오더를 엄격히 검증하는 17개 JVM 단위 테스트 구축.",
        outcome:
          "코드 리팩토링 및 NPU 드라이버 업데이트 시 핵심 비즈니스 로직과 파일 입출력 회귀를 자동 탐지하는 테스트 환경 확보."
      },
      {
