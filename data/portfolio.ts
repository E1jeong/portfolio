...
export const projects: Project[] = [
  {
    ...
    outcomes: [
      "외부 보안 앱(SFX/SIU) 간의 AIDL IPC 연동 규격을 정비하여 수용평가(RQA) 연동 테스트를 통과했습니다.",
      "WPA2-Enterprise(EAP-TLS) 암호화 연결의 인증서 검증 우회 가능성을 점검하고 검증 로직을 강화했으며 로컬 데이터 암호화를 적용했습니다."
    ],
    ...
    result: "고객사 요구 기능을 기존 제품 구조에 통합하여 수용평가(RQA) 연동 테스트를 통과하는 빌드를 확보했습니다.",
    features: [
      {
        ...
        solution: "AIDL 인터페이스 경계면의 예외 처리를 단말 제어 생명주기와 분리하고, 비동기 호출 시 데이터 전달 정합성을 높이도록 Thread-safe한 상태 처리 큐(Queue)와 재시도 로직을 설계 및 이식했습니다.",
        outcome: "수용평가(RQA) 단계에서 단말 제어 연동 테스트를 통과하여, 외부 프로세스 연동 시의 동작 신뢰성을 개선했습니다."
      },
      {
        title: "WPA2-Enterprise (EAP-TLS) 사설 RADIUS 환경 보안 암호화 인증 우회 취약점 해결",
        description: "고안전성 기업용 출입문 Wi-Fi 인프라 연결 시, 표준 TLS 암호화 핸드셰이크를 준수하고 인증서 정합성을 통제하여 중간자 공격(MITM) 및 보안 우회 가능성을 줄이는 통신 보안 강화 작업입니다.",
        ...
        outcome: "사설 RADIUS 접속에서의 인증서 검증 우회 가능성을 점검하고 검증 로직을 강화하여 기업 네트워크 보안 요구사항에 대응했습니다."
      },
      {
        ...
        outcome: "플랫폼 서버 장애나 비정상 데이터 유입 환경에서도 단말이 크래시 없이 안정적으로 구동되도록 런타임 안정성을 개선했습니다."
      },
      {
        title: "MediaPipe Tasks Vision 기반 랜드마크 분석 제스처(Gesture) 오인식 개선 알고리즘",
        description: "비접촉 근태 등록 및 기기 제어를 위해 단말 전면 카메라 입력에서 사용자 손동작(Gesture)을 실시간으로 감지하고 분류하는 온디바이스 AI 비전 알고리즘 보강입니다.",
        challenge: "MediaPipe 모델이 내놓는 단순 분류 결과만으로는 V자 포즈(Victory)와 검지 지목(PointUp) 손동작 간의 미세한 각도 차이를 오인식하여, 잘못된 근태 명령이 실행되는 감지 신뢰도 문제가 발생했습니다.",
        solution: "기본 분류 모델 결과에 더해, 실시간으로 검출된 손가락 관절 랜드마크(Landmarks) 좌표 상에서 각 손가락 끝(Tip)과 두 번째 관절 마디(Pip)의 높이 차이 및 굴곡도를 픽셀 변위 분석으로 수학적 2차 필터링 검증 단계를 설계했습니다.",
        outcome: "Victory와 PointUp의 판단 모호도를 해소하여 제스처 인식 정확도를 개선하였으며, 불필요한 근태 오입력 가능성을 줄였습니다."
      },
      {
        title: "FeliCa IDm 스마트카드 로컬 캐싱 및 대용량 오프라인 매칭 최적화",
        description: "Suica 등 일본 현지에서 널리 쓰이는 FeliCa 규격의 모바일 스마트카드를 단말 리더기가 인식했을 때, 로컬 데이터베이스를 기반으로 단말 자체에서 1대N 검증을 신속히 수행하는 하드웨어 리딩 최적화입니다.",
        challenge: "네트워크 단절 시에도 출입이 가능하도록 대용량 사용자 IDm 리스트를 단말 로컬 DB(SQLCipher)에 저장·매칭해야 했으나, 짧은 갱신 주기로 인해 플랫폼 동기화 시 로컬 I/O 병목 및 단말 UI 스레드 일시 정지(Freezing) 현상이 발생했습니다.",
        solution: "대규모 변경 사항을 데이터베이스에 반영할 때 Room/SQLite 벌크(Bulk) 트랜잭션 처리를 고도화하고, DB 직접 조회를 최소화하기 위해 메모리 단에 최적화된 Lookup 캐시 계층을 배치하여 카드리더 스레드와의 연동 효율을 높였습니다.",
        outcome: "인터넷이 차단된 오프라인 환경에서도 대용량 IDm 리스트를 신속하게 로컬 매칭하도록 개선하고, 플랫폼 동기화 중 발생하던 단말 오작동을 해소했습니다."
      },
      {
        title: "Wiegand 외부 입출력 제어 프로토콜의 패리티 비트(Parity Bit) 이기종 단말 정합성 확보",
        description: "인증 완료 시 외부 도어 컨트롤러 또는 하드웨어 컨트롤 장치로 Wiegand 신호를 송신하거나 리더로부터 카드 번호를 수신할 때, 비트 수준의 신호 정합성을 보장하는 임베디드 통신 최적화입니다.",
        challenge: "이기종 단말 모델(NPro와 XPro) 간에 Wiegand 비트 전송 규격(NPro는 Parity Bit 강제 추가, XPro는 26/34bit에만 Parity 적용 후 32bit raw 전송)이 달라, 타사 컨트롤러 연동 시 카드 ID의 이진 변환 값이 비트 밀림 등으로 깨져 출입문 제어가 무산되는 장애가 있었습니다.",
        solution: "`WiegandComm.sendID()` 및 `sendBinaryString()` 메서드 단에서 26bit, 32bit, 34bit 포맷 패킷을 분석해 홀수/짝수 패리티 비트 연산 로직을 동적으로 분기 적용하고 하드웨어 하위 비트 송수신 레이어의 통신 규격을 정밀 매칭했습니다.",
        outcome: "국내외 다양한 벤더사 출입 통제 컨트롤러 장비와의 물리 비트 호환성 테스트를 통과하여 현장 장비 호환 오류를 해소했습니다."
      }
    ],
